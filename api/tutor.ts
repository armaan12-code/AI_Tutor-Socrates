...
const MISSING_API_KEY_MESSAGE =
  "Missing Gemini API key on server. Set GOOGLE_API_KEY or GEMINI_API_KEY in Vercel Environment Variables.";
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    for (const model of modelOrder) {
      try {
        console.log(`[Vercel Serverless Tutor] Attempting generation with ${model}...`);
        ...
        console.warn(`[Vercel Serverless Tutor] Model ${model} failed:`, error);
        lastError = error;
      }
    }
    ...
    console.error('[Vercel Serverless Tutor] Error:', error);
